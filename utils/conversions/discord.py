...
class StrictRole(FuzzyRole):
    ...
    async def check(self, ctx: Context, role: Role) -> None:
        ...
        if role >= ctx.guild.me.top_role and ctx.bot.user.id != ctx.guild.owner_id:
...
class TouchableMember(MemberConverter):
    ...
    async def check(self, ctx: Context, member: Member) -> None:
        ...
        if ctx.command.qualified_name in ["role", "role add", "role remove", "picperms"]:
            return True

        if member.id in ctx.bot.owner_ids:
            raise BadArgument(f"You're not allowed to **{ctx.command.qualified_name}** a bot owner!")

        if ctx.author == member and not self.allow_author:
            raise BadArgument(f"You're not allowed to **{ctx.command.qualified_name}** yourself!")
