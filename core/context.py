...
ChannelType: TypeAlias = Union[VoiceChannel, TextChannel, Thread]
MessageResponse: TypeAlias = Union[Message, WebhookMessage]
...
class Approve(discord.ui.View):
    ...
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        button.label = await self.ctx.bot.get_text("system.confirm.confirmation.buttons.approve", self.ctx)
        if interaction.user != self.user:
            await interaction.warn(
                await self.ctx.bot.get_text(
    ...
    async def decline(self, interaction: discord.Interaction, button: discord.ui.Button):
        button.label = await self.ctx.bot.get_text("system.confirm.confirmation.buttons.decline", self.ctx)
        if interaction.user != self.user:
            await interaction.warn(
                await self.ctx.bot.get_text(
                    "system.confirm.ONLY_USER_RESPOND",
...
class Context(OriginalContext):
    ...
    async def get_webhook(self) -> Webhook:
        ...
        if self.channel.id in self._webhook_cache:
            return self._webhook_cache[self.channel.id]
        ...
        webhook = webhooks[0] if webhooks else await self.channel.create_webhook(
            name="evict - reskin"
        )
        self._webhook_cache[self.channel.id] = webhook
        return webhook
