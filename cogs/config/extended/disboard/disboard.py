...
from utils.tools.formatter import codeblock, plural, vowel
...
class Disboard(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def bump_listener(self, message: Message):
        ...
        if (
            ...
            or not message.interaction_metadata
        ...
        user = message.interaction_metadata.user if message.interaction_metadata else message.author
        await self.bot.db.execute(
            """
    ...
    @disboard.group(
        ...
        invoke_without_command=True,
        example="Can someone {disboard} the server?",
    )
    @has_permissions(manage_channels=True)
    ...
    @disboard_message.group(
        ...
        aliases=["thanks"],
        invoke_without_command=True,
        example="Thank you {user} for bumping the server!",
    )
    @has_permissions(manage_channels=True)
