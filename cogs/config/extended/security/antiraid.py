...
class RaidIncident:
    def __init__(self, guild_id: int, members: List[Member], punishment: str):
...
async def process_raid_incident(bot, incident: RaidIncident) -> None:
    """Process raid incident data immediately"""
    ...
    await gather(
        bot.db.execute(
            ...
            """,
            incident.guild_id,
            len(incident.members),
            incident.punishment,
            incident.to_dict()["raiders_data"],
            incident.timestamp
        ),
        bot.redis.hmset(
            f"metrics:raids:{incident.guild_id}",
            {
                "total_raids": await bot.redis.hincrby(f"metrics:raids:{incident.guild_id}", "total_raids", 1),
                "total_raiders": len(incident.members),
                f"punishment_{incident.punishment}": await bot.redis.hincrby(f"metrics:raids:{incident.guild_id}", f"punishment_{incident.punishment}", 1)
            }
        )
    )

async def process_raid_mitigation(bot, guild: Guild, ends_at: datetime) -> None:
    """Process raid mitigation actions immediately"""
...
class AntiRaid(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def submit_incident(
        ...
        if guild.id in self.active_raids:
            return  
            
        self.active_raids.add(guild.id)
        ...
        try:
            ...
            incident = RaidIncident(guild.id, members, punishment)
            await process_raid_incident(self.bot, incident)
            ...
            await gather(
                ...
                process_raid_mitigation(self.bot, guild, ends_at)
            )
            ...
            with suppress(HTTPException):
                ...
                embed.add_field(
                    name="**Action**",
                    value="New members & DMs have been temporarily restricted for an **hour**",
                    inline=True,
                )
            ...
            if settings and settings.get("dm_enabled"):
                dm_tasks = []
                for member in members:
                    dm_tasks.append(create_task(self.send_raid_dm(
                        member, 
                        guild,
                        punishment,
                        len(members),
                        settings.get("dm_antiraid")
                    )))
                
                await gather(*dm_tasks, return_exceptions=True)
            ...
            self.active_raids.remove(guild.id)

    async def send_raid_dm(
        self,
        member: Member,
        guild: Guild,
        punishment: str,
        ...
        """Send DM to raid member with timeout"""
        try:
            if not script:
                embed = Embed(
                ...
                embed.add_field(
                    name="Reason",
                    value=f"Detected {raid_size} simultaneous joins",
                    inline=True
                )
                ...
                await script_obj.send(
                    member,
                    guild=guild,
                    reason=f"Detected {raid_size} simultaneous joins",
