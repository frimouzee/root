...
class Appeal(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def appeal_setup(self, ctx: Context):
        ...
        if view.value == "direct":
            ...
            appeal_server = await self.get_appeal_server(appeal_server_id)
            if not appeal_server:
            ...
            member_data = await self.bot.ipc.broadcast(
                "get_member_info",
                {
                    "guild_id": appeal_server_id,
                    "user_id": ctx.author.id
                }
            )
            print(member_data)
            has_admin = False
            for response in member_data.values():
                if response and response.get('administrator'):
                    has_admin = True
                    break
            ...
            channels = await self.create_appeal_channels(appeal_server_id, ctx.guild.id)
            if not channels:
                return await ctx.warn("Failed to create appeal channels!")

            appeal_channel_id, logs_channel_id = channels
            ...
            embed = Embed(
                ...
                description=f"Appeal server setup complete!\n"
                          f"Appeal Channel: <#{appeal_channel_id}>\n"
    ...
    async def get_appeal_server(self, server_id: int) -> Optional[dict]:
        """Get appeal server info via IPC if needed"""
        ...
        try:
            responses = await self.bot.ipc.broadcast("get_guild_info", server_id)
            for response in responses.values():
                if response:
                    return response
            return None
        except Exception as e:
            log.error(f"Failed to get guild via IPC: {e}")
            return None

    async def create_appeal_channels(self, server_id: int, guild_id: int) -> Optional[tuple[int, int]]:
        """Create appeal channels via IPC if needed"""
        guild = self.bot.get_guild(server_id)
        if guild:
            ...
            return appeal_channel.id, logs_channel.id
        
        try:
            responses = await self.bot.ipc.broadcast(
                "create_appeal_channels",
                server_id,
                guild_id
            )
            ...
            for response in responses.values():
                if response: 
                    return response['appeal_channel_id'], response['logs_channel_id']
            return None
        except Exception as e:
            log.error(f"Failed to create channels via IPC: {e}")
            return None