...
def get_font(size: int) -> ImageFont.FreeTypeFont:
    try:
        return ImageFont.truetype("assets/fonts/impact.ttf", size)
    except OSError:
        return ImageFont.load_default()

def optimize_image(img: Image.Image, max_size: Tuple[int, int] = (800, 800)) -> Image.Image:
...
def process_image_effect(image_bytes: bytes, effect_type: str, **kwargs: Any) -> bytes:
    """Process image with specified effect."""
    with tracer.start_as_current_span("process_image") as span:
        span.set_attribute("effect", effect_type)
...
def process_deepfry(image_bytes: bytes, **kwargs: Any) -> bytes:
    """Deep fry image effect with enhanced contrast and sharpness."""
    with tracer.start_as_current_span("deepfry"):
        img = Image.open(BytesIO(image_bytes)).convert("RGB")
        img = optimize_image(img)
        
        for enhancer, factor in [
            (ImageEnhance.Color, 2.0),
            (ImageEnhance.Contrast, 2.0),
            (ImageEnhance.Sharpness, 7.0)
        ]:
            img = enhancer(img).enhance(factor)
        
        return save_image(img, "PNG")

def process_spin(image_bytes: bytes, **kwargs: Any) -> bytes:
    """Create spinning animation."""
    with tracer.start_as_current_span("spin"):
        ...
        return save_animated(frames, duration=50)
...
def save_image(img: Image.Image, format: str = "PNG") -> bytes:
...
def save_animated(frames: List[Image.Image], duration: int = 50) -> bytes:
    """Save animated image with optimization."""
    output = BytesIO()
    frames[0].save(
        output,
        format="GIF",
